...
if ! ismounted /dev; then
    mount -t devtmpfs -o mode=0755,nosuid,strictatime,size=32M devtmpfs /dev >/dev/null
fi
...
if ! ismounted /dev/shm; then
    ...
    mount -t tmpfs -o mode=1777,nosuid,nodev,strictatime,size=4G tmpfs /dev/shm >/dev/null 
fi

if ! ismounted /run; then
    mkdir -m 0755 /newrun
    mount -t tmpfs -o mode=0755,nosuid,nodev,strictatime,size=4G tmpfs /newrun >/dev/null 
